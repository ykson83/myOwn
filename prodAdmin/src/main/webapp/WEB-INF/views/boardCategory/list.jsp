...
<!-- 								</div> -->
	...
	<%@ include file="/WEB-INF/views/boardCategory/modal.jsp"%>
	...
	<script>
		...
		$(document).ready(function() {
			...
			$("#boardCategoryListMenu").addClass("active");
...
// 		        pageLength: 25,
				lengthMenu : [10, 25, 50] ,
		        ajax: {
		            url: "${ctxPath}/api/boardCategory/list_with_count",
		...
		function nameChecking() {
			...
			handleGet("/boardCategory/nameCheck/" + objectId, "", function(result) {
				//TODO: ResponseCode ...변경
				if(result.code == 200) {
					nameCheck = true;
				}
		...
		function save() {
				var obj = makeObj();
				handlePost("/boardCategory/", obj, function(result) {
					alert(result.message);
					location.reload();
				});
		...
		function updModal(objectId) {
			...
			handleGet("/boardCategory/" + objectId, "", function(result) {
		...
		function update() {
			...
			handlePatch("/boardCategory/" + obj.boardCategoryId, obj, function(result) {
				alert(result.message);
				location.reload();
			});
		}
		...
		function del() {
			var boardCategoryId = $('#deleteIdOne').val();
			handleDelete("/boardCategory/" + boardCategoryId, "", function(result) {
				alert(result.message);
				location.reload();
			});
