...
public class BoardCategoryController extends BaseController {
	Logger log = LoggerFactory.getLogger(this.getClass());

	@Autowired BoardCategoryService bCateService;
	...
	public Object listWithCount(HttpServletRequest req, HttpServletResponse resp, DataTableVO reqCond) throws IOException {
		log.info("boardCategoryController/get list_with_count");
		...
		int status = webUtil.parseInt(req.getParameter("status"), 0);
		param.put("status", status);

		webUtil.setPagi(req, reqCond, param);
		webUtil.setDateRange(req, param);

		DataTableVO dataVO = bCateService.listWithCount(param);
	...
	{	
		BoardCategoryEntity _p = (BoardCategoryEntity) bCateService.select(objectId);

		return _p != null ? 
				responseObjectDTO(_p) :
					responseObjectDTO(ConstantContext.RespCode.NOT_FOUND, "cannot find object: " + objectId, null);
	}

	//카테고리 name 중복체크
	//TODO: 이런경우는 내생각엔 존재하는 경우가 오류기때문에 오류 코드를 변경해야 할 거 같음
	...
	{	
		BoardCategoryEntity _p = (BoardCategoryEntity) bCateService.selectByName(objectId);
		
		int respCode = ConstantContext.RespCode.OK;
		String msg = "사용가능한 name입니다: " + objectId;
		
		if(_p != null) {
			respCode = ConstantContext.RespCode.ALREADY_EXIST;
			msg = "이미 존재하는 name입니다"; 
		}

		return responseObjectDTO(respCode, msg, null);
	}
	...
	{
		BoardCategoryEntity _t = mapper.readValue(inputObj, BoardCategoryEntity.class);

		setInitData(_t, false);
		bCateService.insert(_t);
